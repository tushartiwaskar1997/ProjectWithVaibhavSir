...
public class Controller {
    ...
    @PostMapping("/saveemp")
    public ResponseEntity<?> savethestudent(@Valid @RequestBody EmployeeRequestDto empRequeset ){
        ...
        return  new ResponseEntity<>(empserv.savetheemployeeeindb(employee), HttpStatus.CREATED);
        //return  RequestHandler.generateResponse(MessageConfig.SUCCESS_MESSAGE,HttpStatus.CREATED,empserv.savetheemployeeeindb(employee));
            }else
            {
                //return RequestHandler.generateResponse("sdfgs" ,HttpStatus.BAD_REQUEST , null ) ;
                return  new ResponseEntity<>("asdf",HttpStatus.BAD_REQUEST );
            }
    }

    // this type of request is send by http://localhost:8080/base/getbuyid?id=3
    @GetMapping("/getbuyid")
    public ResponseEntity<?> gettheemployeebyId( @RequestParam("id") String  id ){
        Optional<Employee>  emp =  empserv.getbyidemployee(Integer.parseInt(id));
        if(emp.isPresent()){
            return new ResponseEntity<>(emp.get().toString() , HttpStatus.OK);
        }
        return new ResponseEntity<>(MessageConfig.NOT_FOUND, HttpStatus.NOT_FOUND);
    }

    @GetMapping("/deletebyid/{id}")
    public String deletefromempid(@PathVariable String  id ){
        Optional<Employee>  emp =  empserv.getbyidemployee(Integer.parseInt(id));
        if(emp.isPresent()) {
            return empserv.deletefromempid(Integer.parseInt(id));
        }else
        {
            return MessageConfig.NOT_FOUND;
        }
    }

    @GetMapping("/getlist")
    public ResponseEntity<?> getthelistofEmployees(){
    ...
    public ResponseEntity<?>  updateemployees(@RequestParam String id ,@RequestBody EmployeeRequestDto empdto ){
            Optional<Employee>  emp =  empserv.getbyidemployee(Integer.parseInt(id));
            ...
            if(emp.isPresent()){
                ...
                return new ResponseEntity<>(empupdate.toString() ,HttpStatus.CREATED);
            }
        return  new ResponseEntity<>(MessageConfig.NOT_FOUND ,HttpStatus.NOT_ACCEPTABLE);
    }

    @PutMapping("/saveImage")
    public  ResponseEntity<?> AddTheImageToEmployee(@RequestParam("id") String  id , @RequestParam("image") MultipartFile  file)
            throws IOException {
                Employee emp_repose =  empserv.savetheimageofemployee(Integer.parseInt(id), file);
                return new ResponseEntity<>(emp_repose,HttpStatus.CREATED);
    }
    @PostMapping("/saveempusingobjmapper")
    public ResponseEntity<?> SaveTheEmployeeUsingObjectMapper(@RequestParam("pojo") String employee_pojo ,@RequestParam("image") MultipartFile file) throws IOException {
        try {
